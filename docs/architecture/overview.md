...
> 注：此图展示的是每次分派的质量循环。G5（阶段边界）、G6（流水线完整性）和 G7（轮次后审计）分别在 T2 阶段过渡、T3 流水线边界和轮次收尾时触发，处于该逐技能循环之外。
...
> Note: This diagram shows the per-dispatch quality loop. G5 (phase boundary), G6 (pipeline integrity), and G7 (post-round audit) fire at T2 phase transitions, T3 pipeline boundaries, and round closure respectively, outside this per-skill loop.

---

## 评分层级

Shenbi 采用三层测试，单次测试以 **90 分**为通过门槛，**94 分**为阶段推进门槛（94 分源自 `tests/tiers/acceptance.json`，90 分为项目约定），收敛目标为满分 100。三种测试类型各有侧重：generative 生成产出并评分；bug-hunt 检测注入缺陷；clean 审查干净输入，幻觉出的缺陷直接记 0 分。
...
Shenbi uses a three-tier testing scheme, where an individual test **>= 90 is a pass** and **>= 94 is the advancement threshold** (per `tests/tiers/acceptance.json`), with a convergence target of a perfect 100. Three test types each probe a different dimension: generative generates output and scores it; bug-hunt detects injected defects; clean reviews clean input, where a hallucinated defect scores 0.
...
- → [核心概念 / Core Concepts](../getting-started/concepts.md)
