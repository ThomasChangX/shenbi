...
3. **R3 目录内容校验 + 契约对齐**（二选一中取校验，不留静默 SKIP）：gates/g4 增目录纯检查器（幂等无副作用），**检查项按路径参数化**：snapshot 类目录（closure step 10）= 目录存在 + ≥1 产物文件 + manifest 存在（manifest 文件名在本 spec 实现时定死并与 #26 的验收对齐——snapshot-manage SKILL 未约定文件名，两 spec 都依赖它）；非 snapshot 目录（`characters/`，triggers.py:274 卷扩展步骤）= 目录存在 + ≥1 产物文件（**无 manifest 检查**——manifest 是 snapshot 概念，`characters/` 强制它会让每次卷边界扩展 G4 永久 FAIL，在另一路径复刻 R3 失败类）；closure step 10 的 output_path 与 snapshot-manage 契约对齐（`snapshots/chapter-NNN/*` 解析为最终章号目录，或按契约实际产物校验目录），BOOK_CLOSURE artifact 路径同步；与 #8（F163 G4 目录参）衔接面协调注记同 M5 模式。**验收**：closure step 10 在 fixture 项目 G4 PASS；`characters/` 场景（无 manifest）同 PASS
4. **R4 per-step N 语义解析表**（F245 的处置面，单一信源置于 contracts/paths.py）：`arc-N`→N=chapter//12；`stratum-N`→N=chapter//36；`volume-N`→N=边界集中 ≤chapter 的边界计数（注意：**不是** closure `_current_volume` 的 `len(boundaries)`——那只在本卷末章恰好相等，mid-book 会错卷）；`chapter-N`→N=章号。**两个消费者都要接线**：(a) 触发步骤 G4 校验经该表解析；(b) **触发派发侧**——`run_triggered_skills`（triggers.py:561）须从同一张表构造 `{chapter, arc=chapter//12, stratum=chapter//36, volume=count(boundaries≤chapter)}` 上下文传给 `dispatch_skill`，否则 `_build_skill_prompt` 继续按章号解析契约 writes（dispatch_helper.py:653-658）——ch 60 时 prompt 指令写 `arc-60.md` 而修后 G4 查 `arc-5`，「触发不 fire」变「fire 即永久 ESCALATION」。**跨路由通道**：dispatch_skill 三路由中 legacy 子进程路由（`shenbi-dispatch <skill> <type> <dir> <prompt>`，dispatch_helper.py:1826-1865）无 kwargs 通道且在子进程内用 `extract_chapter(prompt)` 重derive——**上下文必须经 prompt 文本载体跨进程**：prompt 追加结构化行（如 `[path-context] arc=5 volume=3 for chapter 60`），派发侧/子进程侧 resolver 优先解析该行、无该行才回落现行章号提取（回落语义不变，向后兼容）；**`_resolve_closure_g4_path`（closure.py:148）是第三消费者**——步骤 6 经表取章号语义（F313 的接线点），不得继续对一切 N 统一按卷号替换。**验收**：chapter 60 arc 场景 G4 查 `arc-5-score.md`（非 arc-60）；**派发 prompt 含 `[path-context]` 结构化行且 "Files to create" 列 `arc-5` 路径；resolver（纯函数单测）优先解析该行、无该行回落章号提取行为不变**（不触发真实 dispatch——核心原则 8，fixtures/单测表达）；卷末章 55 volume 场景查 `volume-3-score.md`；closure step 6 G4 查 `chapter-100-long-span.md`（最终章语义）
