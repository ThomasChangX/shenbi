> **Date:** 2026-08-14 | **Status:** Design（Revised 2026-08-15 · 阶段 3 设计审查多轮收敛中。R2 total=100 修正 / R3 目录校验+契约对齐 / R4 per-step N 语义表+派发侧上下文 / R5 显式解析上下文 / R6 节点·桥接·卷上下文三消费方 / F303 拆至 #26 / F340 重试计数重置 / F341 全守卫体镜像 / F304 捕获定位 / F380 哨兵路径）| **Severity:** 🟥 P0（簇级；成员严重度对齐台账：F324=P0，F353/F371/F373/F379/F340/F341/F304=P1，F245/F380/F3B5=P2）| **方法:** systematic-debugging 四阶段
...
2. **R2 genesis 固化 + 口径统一 + 在途 heal**：total_chapters := max(read_volume_boundaries())（=规划总章数；与 cli.py 卷扩展路径的 `new_total = max(boundaries)` 既有惯例同源）；**双写路径**——(a) genesis step 6（volume-outlining）成功钩子处由代码固化（genesis.py 成功路径，无后续步骤重写 volume_map——已核对 GENESIS_STEPS）；(b) **在途 heal**：cmd_next/cmd_resume 进入 CHAPTER_LOOP 时若 `total <= 0`，先从 volume_map 重算（边界集非空才写）再过守卫——否则已过 genesis 的存量项目（生产实证：56 章、total=None）固化钩子永不再触发，自锁原样保留，`current_chapter` 盲递增写 57…∞。`_count_total_chapters`/`_update_total_chapters` 统一收敛到 `_shared.py`（避免 cli↔genesis 循环 import；签名 `(project_dir) -> int` 走 safe_write，更新模块 docstring 维持 stdlib-only leaf 声明）。**验收**：(i) temp-dir 项目副本 genesis 流程后 novel.json.total_chapters==100；(ii) **中卷 heal**——fixture 项目副本（56 章、total=None）执行 next 后 novel.json.total_chapters==100 且 `check_triggers(55, 100).volume_boundary==True`（heal 后卷边界触发可达；第 56 章态下一个边界是 75，验收用 check_triggers(55,100) 直接断言而非跑到边界章）
3. **R3 目录内容校验 + 契约对齐**（二选一中取校验，不留静默 SKIP）：gates/g4 增目录纯检查器（目录存在 + ≥1 产物文件 + manifest 存在，幂等无副作用）——**manifest 文件名须在本 spec 实现时定死并与 #26 的验收对齐**（snapshot-manage SKILL 未约定文件名，两 spec 都依赖它）；closure step 10 的 output_path 与 snapshot-manage 契约对齐（`snapshots/chapter-NNN/*` 解析为最终章号目录，或按契约实际产物校验目录），BOOK_CLOSURE artifact 路径同步。**同类目录路径**：`characters/`（triggers.py:274，volume-boundary 扩展步骤）走同一目录检查器——与 #8（F163 G4 目录参）有衔接面，协调注记同 M5 模式。**验收**：closure step 10 在 fixture 项目 G4 PASS；`characters/` 场景同 PASS
4. **R4 per-step N 语义解析表**（F245 的处置面，单一信源置于 contracts/paths.py）：`arc-N`→N=chapter//12；`stratum-N`→N=chapter//36；`volume-N`→N=边界集中 ≤chapter 的边界计数（注意：**不是** closure `_current_volume` 的 `len(boundaries)`——那只在本卷末章恰好相等，mid-book 会错卷）；`chapter-N`→N=章号。**两个消费者都要接线**：(a) 触发步骤 G4 校验经该表解析；(b) **触发派发侧**——`run_triggered_skills`（triggers.py:561）须从同一张表构造 `{chapter, arc=chapter//12, stratum=chapter//36, volume=count(boundaries≤chapter)}` 上下文传给 `dispatch_skill`，否则 `_build_skill_prompt` 继续按章号解析契约 writes（dispatch_helper.py:653-658）——ch 60 时 prompt 指令写 `arc-60.md` 而修后 G4 查 `arc-5`，「触发不 fire」变「fire 即永久 ESCALATION」；**`_resolve_closure_g4_path`（closure.py:148）是第三消费者**——步骤 6 经表取章号语义（F313 的接线点），不得继续对一切 N 统一按卷号替换。**验收**：chapter 60 arc 场景 G4 查 `arc-5-score.md`（非 arc-60）**且派发 prompt 的 "Files to create" 列 `arc-5` 路径、落盘产物为 `arc-5-score.md`**；卷末章 55 volume 场景查 `volume-3-score.md`；closure step 6 G4 查 `chapter-100-long-span.md`（最终章语义）
5. **R5 显式路径解析上下文**：dispatch/prompt-build 增可选 context 参数（占位族→值；缺省回落现行 prompt 文本提取，向后兼容）。closure per-step 表：步骤 2（memory-distill）→**最终弧号 = total_chapters//12（=8）**（ch 85-96 周期已写过 arc-8——closure L2 覆写为 85-100 综合是**预期行为**，声明之）；4/5（review-arc-payoff/score-volume）→卷号；6（review-long-span）→**最终章号**（=total_chapters，F313 同修）；10→契约目录。F3B5（genesis escalation）传入显式上下文（失败章号或 book 级哨兵）——**哨兵值须与 genesis checkpoint artifact 字符串同步**（现为 `audits/escalation-genesis-report.md`，genesis.py:260，而契约是 `escalation-N-report.md`——两处必须一致，否则人工被指向技能永不写的路径）；F380（anchor-curate）`AC-NNN` 用 genesis 表既有 book 级哨兵形态（`AC-001.md`，genesis.py:78）——**不得仅从 output_paths 剔除了事**（剔除后 "Files to create" 为空 → "No output files written" → optional 跳过原样复现），跳过须留 structlog 原因。**验收**：closure 10 步 prompt-build 全通过（fixture）；escalation 派发不抛 UnresolvedPathError；anchor-curate 真实派发（prompt 含具体 AC 路径）而非伪装 optional 跳过
6. **R6 中文节点/桥接/卷上下文提取**：`_shared` 增章节节点提取（`| 第N章 |` 表行）、桥接提取、卷头/卷目标双语识别；**三处**节点消费方全部接线去重——chapter_loop.py:2182、plan_skeleton.py:197、context_assemble.py:243（第三处内联 `chapter_node_pattern`，漏接则上下文装配路径污染原样保留）；第四处 `volume_align.extract_chapter_node`（英文 `## Chapter N` 模式，仅测试引用零生产调用方）**显式移交 #16/#25 死代码批次处置**，R6 不接线它。桥接**段级**：生产 map 有**五个** `### 跨卷桥接` 表（各卷一个）——共享提取器须**聚合全部桥接段**（现 `split(...)[1]` 只吃第一卷的表，4/5 桥接永不浮现）；段定位双语（`### 跨卷桥接` / `## Cross-Volume Bridges`），context_assemble.py:254 / plan_skeleton.py:208 切分同步。桥接**行级语义**（6 列中文表 `| # | 钩子内容 | 类型 | 带入卷 | 预期激活章 | 当前状态 |`，激活值含区间 `第26-28章` 与非数值 `展开主线`）：激活章=区间最小值；非数值行跳过 + structlog WARN；**带入卷非本书卷的行（续作行，如 vol-5 表的 `《星火燃穹》续作` + 激活 `第1章`）跳过**——否则续作钩子从第 1 章起污染每章上下文。卷上下文：`_resolve_volume_at_runtime` 的 `Volume {i}` 命名与 context_assemble.py:226-240 的 `## Volume N`/`**Objective:**` 匹配补中文（`## 第N卷`、`**Objective**:` 冒号在粗体外）。**验收**：生产 map 第 5+ 章节点非 None 且 role/content 非桥接表垃圾；**vol-1 桥接**：激活窗内的章（如第 26 章）出现 vol-1 表桥接条目；**vol-2 桥接**：第 36 章出现 vol-2 表（激活 `第36章`）的桥接条目、第 30 章不出现；**续作行**：第 1-10 章上下文不含 vol-5 续作桥接；卷上下文块在中文项目非空
7. **F340 REJECT 重做语义**：cmd_review REJECT → 重做产生该 checkpoint 的步骤。**逐类型语义表**（现 MODIFY 分支 cli.py:543-548 已有 CHAPTER_MEMO/STATE_SETTLE/GENESIS_COMPLETE 的回退游标先例，REJECT 对齐补全；VOLUME_BOUNDARY→重跑卷边界步骤、ESCALATION→重跑失败步骤、PER_CHAPTER→重跑当章评审、GENESIS_COMPLETE→genesis 状态重置到最后一步重跑——`current_step` 回退但 `skills_done` 追加不去重是无害的（消费方仅查存在性，phase_runner.py:280））。**REJECT-redo 须重置该步骤的重试计数**（retry_counts + retry_budget_consumed 对应键）——否则 ESCALATION 类 redo 立即再次耗尽重抛（计数已过 max_revision_retries/max_audit_retries）。**验收**：模拟 genesis-complete reject 后 resume 重跑 step 17 而非 no-op（单测）；escalation reject-redo 后步骤获得完整重试预算再执行（单测）
