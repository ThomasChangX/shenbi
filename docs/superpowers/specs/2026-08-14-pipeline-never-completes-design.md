...
7. **F340 REJECT 重做语义**：cmd_review REJECT → 重做产生该 checkpoint 的步骤。**逐类型语义表**（现 MODIFY 分支 cli.py:543-548 已有 CHAPTER_MEMO/STATE_SETTLE/GENESIS_COMPLETE 的回退游标先例，REJECT 对齐补全；VOLUME_BOUNDARY→重跑卷边界步骤、ESCALATION→重跑失败步骤、PER_CHAPTER→重跑当章评审、GENESIS_COMPLETE→genesis 状态重置到最后一步重跑——`current_step` 回退但 `skills_done` 追加不去重是无害的（该列表**无逻辑消费方**，仅 state.py 序列化读写，state.py:236/328））。**REJECT-redo 须重置该步骤的重试计数**（retry_counts + retry_budget_consumed 对应键）——否则 ESCALATION 类 redo 立即再次耗尽重抛（计数已过 max_revision_retries/max_audit_retries）。**验收**：模拟 genesis-complete reject 后 resume 重跑 step 17 而非 no-op（单测）；escalation reject-redo 后步骤获得完整重试预算再执行（单测）
