...
- **T3 · 数据丢失级特修（F868/F1101/F1104/F1105）**：volume-consolidation/state-settling/摘要与趋势类技能全部走 T2 路由；修订技能对正文文件一律禁写（只产 diff/摘要到侧车）。
  ...
  - **T3 显式输入（2026-08-23 spec #7 REJECT 补录，R5 残留证据）**：修订写路径防御现状——`_check_content_size_guard`（dispatch_helper.py:944-1005）是写前阻断但阈值仅 20%（新内容 < 原文 20% 才拒写），≥20% 的摘要覆写仍无拦截（事后 check_prose_leakage 仅 WARN 不阻断）；「修订技能对正文一律禁写」落地时此窄口子一并消除，且 chapter_loop.py:2730-2733 调用点注释仍称 "disjoint files ... zero data conflict"，与 run_parallel_post_draft_steps docstring 的 known-issue 自述矛盾，T3 修 staging 双写者时同步更正。
