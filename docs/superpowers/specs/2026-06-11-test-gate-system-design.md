...
| GR.4 | 文件系统中存在但不在任何 subagent 声明产出列表中的文件 → 标记 orphan_file → 警告 |
...
| 脚本 | 第 1-3 章（N≤3）：第 1 段含"三面墙"或等价世界观建立描述（ch1）、第 2 段含对手建立（ch2）、第 3 段含小高潮/大主线钩子（ch3） |
| 脚本 | 第 4 段含 `### 关键抉择过三连问` |
| 脚本 | 第 5 段含 hook 账（open/advance/resolve/defer 四种操作名至少出现一种） |
| 完整 | 章尾改变具体可验证 |
| 完整 | "不要做"列表可操作 |
...
| # | 检查项 | 失败行为 |
|---|--------|---------|
| G4.b1 | 产出报告命中场景声明的 planted defect。匹配算法：(a) 文件路径精确匹配，(b) defect 描述提取 3-5 个关键词（名词和动词），报告 finding 中至少包含 2/3 关键词（大小写不敏感的中文子串匹配），(c) severity 标签与场景声明一致 | Kill switch → 总分 0 |
| G4.b2 | 未报告场景中没有的缺陷（零误报：报告中的 finding 数 ≤ 场景声明的 planted defect 数） | 每次误报 -20 分 |
| G4.b3 | 每个 finding 标注 severity（error/warning）和 evidence location（文件:段落引用） | 缺失 → 0 分 |
...
4. **分歧处理**：如果同一文件的两次独立评分（不同评分 subagent）分差 > 15 分，触发第三次评分（仲裁评分）。取两次最接近的分数平均值。
...
| G6.7 | 章节中引用的每个角色名可在 character_matrix 中找到（提取章节正文中所有首字母大写的双字/三字中文名，与 character_matrix 交叉比对） | 孤儿引用 → 扣分 |
...
progress.json 中的 `completed_skill_names` 是已完成技能名的数组（供 GD.1 做 set 比较）。`subagent_completion_count` 是每次 subagent 成功返回的递增计数器（供 G_RECONCILE 触发和 G_DISPATCH 看门狗使用）。`remediation_attempts` 为 `{skill_name: count}` 映射，追踪 G2.6 等自动修复的重试次数。
