...
- 新增 fixtures 载体合规且 g0_purity 不新增违规：truth 类为**单块合并式** frontmatter（`provenance: upstream-copy` 与原键同块；g0_purity `load_provenance` 对 `.md` 只读 frontmatter——sidecar 对 `.md` 无效）、无 frontmatter `.md` 为前置单块、novel.json 为 `.provenance.json` sidecar；验证命令 `uv run pytest tests/unit/gates/test_g0_purity.py tests/unit/gates/test_g0_purity_enforcement.py -v` 全绿（载体/变体旁路面零新违规）；回退路径的 synthetic-sample 引用在测试内注记局限
