> **Date:** 2026-08-16 | **Revised:** 2026-08-31（v2：价值门删 F376/F604；v3：阶段 3 设计审查 1C/3I/5M 修订；v4：route_block 权威冲突裁决等二轮修订；v5：anti-ai 活面重定向等三轮修订；v6：校准覆盖落点单元格 patch 等四轮修订；v7：cell-patch 新原语等五轮修订；v8：HitRate 判对标准与冷启动规则等六轮修订；v9：验收 grep 加 skills/ 面、预算 fixture 漂移与 T4 能力清单边界注记）| **Status:** Done (PR #127) | **Severity:** 🟠 P1 | **方法:** systematic-debugging 四阶段
...
- **T1b · 派发后强制（仅 calibration）+ route_block 死模型删除**：前置——review-resonance 产品契约新增机器可解析的 confidence 与锚点判定字段（写入其 decisions sidecar / 报告结构，走契约三源同步）；框架在解析→落盘路径上从累计的 resonance 历史计算 HitRate，调用 `calibrate_confidence` 复算并**覆盖**落盘的 confidence 值（v7 机制细化：trend 行经 `write_truth_file(mode="insert_markdown_row")` 是 insert-only——技能自写富行存在时框架占位行被跳过，这是**正常生产路径**）——落点为：解析后对 truth 中该章 trend 行（无论技能富行还是框架占位行 wins）做**置信度单元格级 patch**。注意 truth_io **无既有单元格 patch 原语**（`upsert_markdown_row` 整行替换、`_upsert_markdown_bullet` 仅 bullet 格式）→ 本任务新增 `patch_markdown_table_cell` 原语：读行→按列定位 confidence 单元格→splice→回写该行；**列定位语义（v8 钉死 positional）**：一律按位置定位（confidence 为第 8 列 cells[7]）——技能 `append_dedup` 只追加行、表头是否首行存在不受控，表头分支是 dead-wire 风险，弃用；技能富行与框架占位行同为 9 列布局，位置一致；行 cells 数不足 8 时先按既有列结构补齐占位再 patch；行缺失时写框架占位行再 patch。原语走既有 per-path 锁注册表（truth_io.py:55-77，与 insert_markdown_row 同理），禁止裸 read/splice/write。校准前后值记录：structlog 事件（before/after/reported）+ 不引入新 sidecar——**锚点判定与校准历史的结构化累积走 keyed truth 行**（如 `truth/resonance_anchors.md`，每章一行锚点判定结果，keyed upsert 既有设施，走契约声明与三源同步），HitRate 从该行集合计算（分子=高置信锚点判对数、分母=高置信锚点总数，多章累积）。**判对标准与冷启动（v8）**：判对以框架可核的交叉信号为准——取该章后续修订是否实际改写了锚点覆盖的文本块（revision 记录交叉核对），**不用**评分技能自报（自报=循环论证）；锚点累计分母 < 3（证据不足）时 fail-open：不降级、structlog 披露 `calibration_insufficient_history`；不扩展 decisions schema（其枚举封闭，校准记录不适配 selections/adjustments/budget 三段）。不改写 LLM 已过 G2/G4 校验的报告 markdown，顺序在解析后、下一章派发前（LLM 自报值降级为参考）。技能契约同步修订：review-resonance SKILL.md 的 trend 行写入声明注明 confidence 单元格由框架校准后 patch，技能写自报原值即可；锚点判定结果按 truth 行格式追加。校准历史 fixture：无现成多章锚点真实产物，用既有 resonance fixtures 驱动上游生成路径产出（G0.9 禁手写）。routing 半按背景节裁决：**删除 route_block 重复模型**——删除面（v5 补全）：`review_resonance/routing.py`、`review_resonance/__init__.py` 的 re-export（route_block/Routing/RevisionLoop/BORDERLINE_BAND/MAX_AUTO_REVISIONS）、`review_resonance/__main__.py` **整文件**（它只是 routing.main 的裸入口，无子命令结构）、`skills/shenbi-review-resonance/SKILL.md` 中 routing 全部绑定面（v6 扩全）：铁律3（:64-66：`python -m` 指令，且 :64 同句还绑定「§5.4 分流、修订上限」须一并改写——routing/上限删除后该句只剩 confidence 校准为框架自动执行）、铁律5「降级后的置信度才用于 §5.4 分流」（:68）、DOT 流程图「§5.4 分流 (确定性 routing)」节点（:81-82）、阻断规则「按 §5.4 三路径分流」（:114）、「置信度守护与分流（§5.4）」整节（:116 起）——routing 指令随死模型删除，confidence 校准叙述改为「框架自动执行」并注明 trend 行 confidence 单元格由框架 patch、`tests/unit/skill_utils/test_routing.py` 与 `test_confidence_routing_integration.py` 同步处置——后者含 confidence（保留面）用例，拆分后仅删 routing 用例、confidence 用例随 T1b 接线改写为框架路径测试保留。revision_router 保持唯一权威。**修订上限迁移（v7，v8 语义钉固）**：`MAX_AUTO_REVISIONS=2` 目前只存在于待删的 routing.py（:48,97）与待删的 SKILL.md §5.4 叙述——删除后防无限修订循环的保证将整体消失 → 上限检查迁入 `pipeline/revision_router`（框架侧消费 `cs.revision_count`，chapter_loop.py:1831 递增点旁比较，超限 escalate），阈值常量随迁。**语义对账**：`cs.revision_count` 对一切非 NO_REVISION 路由递增（含审计 BLOCKING），比原 routing.py 只计共振驱动修订更宽——明确接受该收窄（任何成因的修订都受同一上限保护）；与既有 `max_audit_retries=3` 的关系：retries 计连续失败重试、cap 计累计修订次数，二者正交不竞争，先到先触发，plan 中以对账测试钉死。
