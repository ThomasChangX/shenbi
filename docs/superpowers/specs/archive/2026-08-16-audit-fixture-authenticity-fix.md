> **Date:** 2026-08-16 | **Status:** Done（PR #174, 2026-09-07; Revised 2026-09-07 — 设计审查 2C+6I+4M 吸收：provenance 载体定义、WARN→FAIL 滚出机制、F750/T807/T808 归属、T4 验收、真实样本源、负样本位置、白名单枚举、证据行文法）| **Severity:** 🟥 P0（F751 内容级断链）
...
6. 伪造快照族（F779/F780）：chapter-025 manifest 占位 checksum → 用真实 novel-output 快照重建；镜像对登记 MIRROR_MAP 或去重（F781 的 32 副本同步守卫一并为 check_fixture_mirror 加 CI 接线——与 C25 F1012 协同；接线时更新 check_fixture_mirror.py:12 的过期注释「提模块级后生效」——MIRROR_MAP 已在 g0.py 模块级；AC2/AC3 的**闭包扫描与 hash 去重脚本落位 tools/**（与基线生成器同侧，gate 内只做判定））
