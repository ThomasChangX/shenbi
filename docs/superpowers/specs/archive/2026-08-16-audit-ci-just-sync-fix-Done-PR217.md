...
证据分组（22 条，2026-09-13 main HEAD b00f564b 亲证存活）：
- **双向清单漂移（P1 核心）**：
...
- **杂项（存活）**：F957（test_docs_accuracy 白名单仅 8 份根级文档（tests/integration/test_docs_accuracy.py:17-26）、CODESPAN_PATTERN 只匹配无空格单 token——防线面与 C17/C23 分工对齐）、F1039（just clean（justfile:110）`rm -rf tests/coverage/` 删跟踪文件 `tests/coverage/.gitkeep`）
...
### T1 · 清单单源化（P1 核心）
1. 重构为 **CI 调 `just check`**（推荐：ci.yml lint/test 面收敛为安装 + `just check`，清单只在 justfile 一处）；或次选——单一被两边引用的 check 清单文件。裁决落文。CI 收敛时**三个结构问题必须一并裁决落档**：(a) quality job 的 os×python matrix 与 `uv python install` 版本钉扎如何保留（matrix 维度在 CI 侧、命令体在 just 侧，或 just 参数化 target）；(b) codegen-idempotency job 三步并入 just check 后该 job 删除还是保留（重复跑幂等 diff 的成本权衡）；(c) runner 上 `just` 的安装步骤
2. F005：`uv lock --check`、autocheck docs 幂等 diff、plugin manifests 幂等 diff 进 just check（与 CI codegen job 对齐）；F1003/F911：`.codex-plugin/` 从 gitignore 放开（幂等 diff 需基线在库；先测体积）或改哈希对比（基线哈希文件入库与否及其体积与方案 A 同构性一并裁决）——二选一裁决落文
3. AGENTS.md「等价命令」改述为「运行 `just check`」或修正为真实等价（含 lint 面 + `-m "last"` 第二段 + lock 检查）
4. 登记新 lint 接入规则（后续新检查只加 justfile 一处，CI 结构性继承）
5. **pytest 旗标统一**：ci.yml:77-78 的 `--dist loadscope --timeout=120` 与 justfile check 段（无 loadscope、超时依赖 addopts `--timeout=60`）的漂移一并收编——CI 调 just 后测试语义不得改变（旗标迁入 justfile 或显式参数化）

### T2 · coverage 工件隔离
5. addopts 拆分：全局 addopts 移除 `--cov/--fail-under`，仅覆盖专用入口显式携带；或常态 `COVERAGE_FILE` 隔离 + collect-only 豁免——最小侵入裁决落文（F770 随此消解）。**影响面五入口全列**：just check 的 pytest 段、pre-push（pre-push-check.sh:70 `--cov-fail-under=85`）、`just module-coverage`（justfile:43-45，现完全依赖 addopts 的 `--cov=shenbi`）、ci.yml 覆盖步骤（ci.yml:76-81，同依赖）、nightly.yml:41（`pytest … --cov-fail-under=85`，同依赖——拆分后不补 `--cov` 则该门静默失效）——后三处在拆分后必须显式携带 `--cov` 否则静默零覆盖。**另**：`-m "last"` 阈值测试（tests/unit/test_coverage_thresholds.py）读取 addopts xml 报告产出的 `tests/coverage/coverage.xml`，拆分后该输入的生产路径必须保留在覆盖专用入口；**验收 2 的 sha256 不变断言仅在方案 A（addopts 拆分）或方案 B 同时禁用 `--co` 阶段报告写出时成立**——两方案与验收兼容性一并裁决落文
...
1. **单源化结构断言**（CI 调 just 方案下 grep 双侧 diff 必然非空——lint 文件名只出现在 justfile——故改用结构断言）：`.github/workflows/ci.yml` 的检查步骤以 `just` 调用呈现、且 ci.yml 中不残留 `uv run python (tools|scripts)/lint_*.py` 形态的直接 lint 调用（`grep -cE 'uv run python (tools|scripts)/lint_' .github/workflows/ci.yml` = 0）；`grep -cE 'uv run (python (tools|scripts)/|pytest)' .github/workflows/ci.yml` 的残余计数与豁免清单（matrix 维度、just 安装等结构性步骤）逐项对照进 PR；justfile 的 check 清单为唯一清单源（人工摘录对照进 PR）。次选方案（共享清单文件）下改为：两侧均引用同一清单文件、无本地副本
2. 覆盖隔离复验（内容级——`tests/coverage/` 除 .gitkeep 外被 gitignore，git status 断言恒绿不可用）：记录 `tests/coverage/coverage.xml` 的 sha256，`uv run pytest --co -q` 两次后 sha256 不变（D101 红转绿）；`just test` 在全过测试上退出码 0（F1040 红灯验证）；覆盖专用入口产出的 coverage.xml 仍使 `-m "last"` 阈值测试绿
...
## 簇成员清单（22 条，REWRITE 后自查用——F911 与 F1003 同根、裁决合并但分开记账）
