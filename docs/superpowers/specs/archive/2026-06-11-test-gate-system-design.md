...
| G2.7 | 章节文件：字数 ≤ ceiling (10000)。重要章（定义：volume_map 中标注为 爆发段/卷首/卷末/高潮 的章节，或 chapter-N-plan.md 中第 1 段标注"重要章"）允许达到 ceiling。非重要章（未标注的普通章节）上限 = floor × 1.5（= 4500 字） | 标记需压缩 |
...
| G2.12 | 文件内容完整：Markdown 文件以合法标题或以句末标点（。！？…）」』"）结束；JSON 文件以 `}` 或 `]` 结尾。注："不以半个句子结尾"的语义判断由评分 subagent 在完整检查中执行，此脚本检查仅验证结构完整性 | 标记失败 |
...
| GT.5 | 下一 test_cycle_phase 所需的所有输入文件存在。检查规则：(a) T1→T2 和 T2→T3 的 skill_group 切换——按 deps.json `expected_outputs` 检查；(b) generative→bug-hunt、bug-hunt→clean、clean→T2——当前 test_cycle_phase 所有已评分技能的产出文件均存在且通过 G2 检查 → PASS（bug-hunt/clean 的输入是前一个 test_cycle_phase 的产出文件，clean→T2 的输入是 bug-hunt 和 clean 两阶段的评分报告和产出文件） | 列出缺失 → 拒绝 |
...
| # | 检查项 | 失败行为 |
|---|--------|---------|
| GD.1 | `set(progress.completed_skill_names) == set(framework_skills_for_current_test_cycle_phase)`。framework_skills_for_current_test_cycle_phase = skills/ 目录下所有技能名（当前设计假定所有 59 个技能均支持 generative/bug-hunt/clean 三种测试类型）。如果未来有技能不支持某测试类型，该技能应在 rubric.md 中标注 `test_types: ["generative"]`，GD.1 自动排除未标注当前 test_cycle_phase 的技能 | 列出未派发技能 → 拒绝 test_cycle_phase 完成声明 |
...
| # | 检查项 | 失败行为 |
|---|--------|---------|
| G4.b1 | 产出报告命中场景声明的 planted defect。场景可声明 1-N 个 planted defect。匹配算法：(a) 文件路径精确匹配，(b) 每个 defect 描述提取 3 个关键词（名词和动词），对应的报告 finding 至少包含其中 2 个（中文子串匹配），(c) severity 标签一致。触发条件：任意一个 planted defect 未被命中 → Kill switch，总分 = 0。全部命中 → 此检查 PASS | Kill switch → 总分 0 |
...
| G5.2 | handoff 完整性检查：技能 N+1 的 Reads 列表中的每个文件路径，必须在技能 N 的 Writes+Updates 列表中可找到（展开通配符后精确匹配）。不检查反向（N 写了但 N+1 没读的额外文件是合法的——技能可能产出辅助文件） | 列出缺失 → handoff 断裂 |
...
| # | 检查项 | 失败行为 |
|---|--------|---------|
| G6.1 | 实际生成章节数 ≥ ceil(expected_chapters × deps.json.t3-pipelines.long-form.min_chapter_ratio) | 拒绝 Pipeline 评分 |
| G6.2 | 章节文件序列无断号（ch1→ch2→ch3 连续，按 `chapters/chapter-N.md` 文件名中的数字排序） | 缺失章 → 标记 gap |
| G6.3 | 每一章通过 G4 chapter-drafting 脚本层级检查 + G2 章节相关子集（G2.6 字数、G2.8 PRE_WRITE_CHECK、G2.9 POST_WRITE_SELF_CHECK、G2.12 文件完整性）。G2.10（模板检测）和 G2.11（truth 文件.bak 对比）不在章节级重复执行 | 失败章列表返回 |
...
| G6.7 | 章节中引用的每个角色名可在 character_matrix 中找到。提取算法：(a) 从正文提取所有 2-4 字连续中文序列（不含标点），(b) 排除停用词（停用词列表：`tests/fixtures/stop_words_zh.txt`，包含常见非人名词汇如"忽然/因为/所以/但是/那里/自己/什么"），(c) 将候选名与 character_matrix 中记录的角色名列表做匹配——先精确匹配，精确未命中则检查候选是否包含角色名或角色名包含候选（双向子串），精确命中优先于子串命中。凡两种匹配均未命中者 → 孤儿引用 | 孤儿引用 → 扣分 |
...
| G7.7 | CHANGELOG.md 已追加本轮条目。文件存在且可写 → 自动追加后 PASS。文件缺失或不可写 → FAIL（需手动创建） | 自动追加或 FAIL |
...
progress.json 中的 `completed_skill_names` 是已完成技能名的数组（供 GD.1 做 set 比较）。**跨 test_cycle_phase 行为**：`completed_skill_names` 在每次 test_cycle_phase 切换时重置为空数组（由 G_TRANSITION 的 GT.1 触发后，round-exec.sh 执行重置）。每个 test_cycle_phase 独立追踪其完成进度。`subagent_completion_count` 是每次 subagent 成功返回的递增计数器（全局不重置，供 G_RECONCILE 触发和 G_DISPATCH 看门狗使用）。`output_files` 是每个技能产出的文件路径数组（供 GR.4 做 orphan 检测）。`remediation_attempts` 为 `{remediation_type: count}` 映射（如 `{"word_count_expand": 2}`），存储在每个技能的 entry 中，追踪 G2.6 等自动修复的重试次数。

**规则**：此文件由 `validate-gate.py` 和 `round-exec.sh` 写入。Agent 只读。

**并发安全**：progress.json 的写入必须使用原子操作（写入临时文件 + os.rename）。`round-exec.sh` 写入前需获取 `progress.json.lock` 文件锁（与 G1.5 使用相同的文件锁协议，参见"文件锁规范"小节）。同一时刻只有一个写入者持有锁。
...
**中文停用词列表**：位置 `tests/fixtures/stop_words_zh.txt`。每行一个停用词（常见非人名词汇如"忽然/因为/所以/但是/那里/自己/什么/已经/可以/没有"）。由项目维护者手动维护。G6.7 使用此文件过滤候选角色名。如果文件缺失 → G6.7 跳过停用词过滤步骤（所有 2-4 字中文序列作为候选，假阳性率升高但不阻塞）。
