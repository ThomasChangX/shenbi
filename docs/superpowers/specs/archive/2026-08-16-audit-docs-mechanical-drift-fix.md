...
- **占位/杂项**：F804/F854（.gitkeep 在已有内容目录中冗余）、F953（command-to-give.md 空节）、F955（spec 预写目录漂移）、F970（coverage-ledger 锚点指向不存在报告）、F971（覆盖率数值与 coverage.xml 不一致）、T1003（台账文本 12 vs 13 patch 笔误）；F006/F909/F910 归证据分组外的补登成员（计数/kind 族与 basedpyright 文档），F1033 为 F904 计数族合并域
...
5. gates 计数（F423）、kind 计数（F907）、G0.12/注册表计数（F440）、归档计数（F967）、复选框回填（F974——**裁决：归档 plan 豁免回填**，历史记录不重写，以 deviation 记录不回填决定；仅活跃文档回填）、覆盖率阈值自述（F1037→对齐 pyproject 实值）、F971/F970 数值锚点重算或删除
6. F769：doc_links 自身文档去掉"371 项"类快照计数，改动态描述

### T3 · 行号锚点降险
7. T1001/F331/F426：spec 与 docstring 中的 `file.py:NNN` 引用尽量改为 `file.py::<symbol>`（函数/常量名）形式；无法符号化的加"行号为 2026-08-16 快照"标注；归档 spec 只在 doc-links 可达性上修，不逐行追号（追不动是 T1001 的教训）。执行期新登：hooks.py:6,31（"``TRIGGER`` at SKILL.md:87" → 改符号/实际锚点）。F425/F460 已剔除（2026-09-13）
8. D103/F117/F127/F334/F335/F428/F429/F126/F612/F909：docstring 过期注释逐条更新（多数为删一行或改一个数）；F909 = docs/basedpyright-overrides.md 三处失实声明（warning 实为 none / skill_utils executionEnvironment 已不存在 / mypy 镜像声明）改写对齐 pyproject 实值
9. **G0.13 强制重锁**：凡改动 `src/shenbi/*.py` 的 task（T1.2/T3.8 涉及），commit 前必须 `bash tests/lock-tool-hashes.sh` 更新 deps.json 哈希，否则 G0.13 阻断

### T4 · 防线接线（与 C17 T2 合流）
10. internal-links 检查已进 per-PR CI（PR #191 落地），本簇 T1/T2 修复以该检查绿灯为机械验收
11. 新增一条最小"文档计数哨兵"（可选）：对保留数字的声称（若 T2 选择保留路线）做 CI 校验——默认走 C22 R1，不建第六张表

### 批量清理（M 级成员，随 T1/T3 批量 commit 顺带清，此处列 ID 备查对账）
D103、F117、F127、F236、F334、F335、F423、F426、F428、F429、F440、F461、F714、F804、F854、F953 —— 均为单行/单词级修订（F955 载体已归档，移入 F968 归档可达性批处理，不出现在此活跃清单）（上表已并入各任务项；F425/F460/T1003 已剔除不出现在此清单）

## 验收标准（真实数据可复验）

1. internal-links 检查（C17 T2 落地后）对全仓 docs+README+command-to-give 跑批 0 断链；修复前基线计数留档对照
2. 断链 grep（活跃入口文档清零口径；ADR-0009、src/ docstring 沿革注、audit-runs/specs/plans 归档区为历史记录，不在命令 scope 内且明文豁免）：`grep -rn "dispatch-subagent" command-to-give.md AGENTS.md README.md` 0 命中；`grep -c "validate-gate" command-to-give.md` = 0（:24 注意事项行随 T1.1 改写为现行工具名，lock-tool-hashes 协议本身保留）；`git grep -n "69 total\|67 functional" -- AGENTS.md README.md docs/framework/` 0 命中（去数字化路线）
3. 抽查行号锚点（T1001/F331/F426 与 hooks.py 锚点，每处引用目标符号可 `grep -n <symbol>` 命中且唯一；F425/F460 已剔除不再抽查）
4. `just check` 全绿；归档 plan 32 条路径用 basename 解析验证 0 失败（F968 红转绿记录；解析脚本于 T1.3 执行期编写为一次性 codemod 自带校验，不新建常驻 tools/ 脚本）
...
- **回滚**：文档/docstring 改动不触生产逻辑，但 src/shenbi docstring 改动会变更 G0.13 哈希锁——每此类 commit 须同步重锁 deps.json（tests/lock-tool-hashes.sh）；分 T1/T2/T3 三批 commit 各自可回滚

## 簇成员清单（自查用）

**活成员（31 原始 + 2 新登锚点 = 33；口径：46 − 初修剔 8 − 执行期剔 5 − 合并 1（F1033 并入 F906 残留，同一处载体）− 归档豁免 1（F955 载体已归档，随 F968 同批只修可达性））**：D103, F117, F126-F127, F236, F331, F334-F335, F423, F426, F428-F429, F440, F461, F612, F714, F804, F854, F901, F906, F909, F917, F951-F953, F968, F970-F971, F974, F1034, T1001 + 执行期新登：hooks.py:6,31 SKILL.md:87 锚点漂移（F953 复裁恢复 2026-09-13：command-to-give.md「### 第二步：确认进度」空节实存）

**执行期二次剔除（5，阶段 2 事实核实 2026-09-13）**：F006（kind 已回 15）、F007/F904/F1023（计数族 AGENTS/README/index 已修或去数字化）、F910（mypy ignore_errors 已清）

**已剔除（8，2026-09-13 驳斥复核；「0 命中」均指活跃区/src，归档区/audit-run 历史记录豁免）**：F638（recall.py 整文件已删，主张 moot）、F907（registry docstring 已改 16）、F967（plans/INDEX 114==114 已同步）、F1037（活跃文档已统一 85，"78" 仅存归档）、F769（"371 项" 仅存归档）、F425（g3_independence 现锚点 :144-162 准确无需动）、F460（"SKILL.md:125" 活跃区/src 0 命中）、T1003（活跃区 0 命中）。F953 初判剔除后复裁恢复（空节实存）

## 2026-09-13 修订注记（价值门驳斥复核）

- **T2 计数治理大半已被 PR #207（C22）消化**：AGENTS.md/overview.md 去数字化已完成，计数族仅剩 `command-to-give.md:85`「全部 59 个 skill」一处（磁盘 74）——T2 收窄为该残留 + F423（gates.md:3 称 8 实际 cli 11）+ F440（g0.py:711 "20 skills"）+ F974（复选框回填，**已裁决：归档 plan 豁免**，见 T2.5）+ F970/F971（执行期核实载体是否仍存在）
