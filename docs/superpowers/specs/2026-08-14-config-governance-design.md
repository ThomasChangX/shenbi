...
## 测试与 fixture 计划
- 基线：`tests/fixtures/genre-config-example.json`（真实产物副本，已在库）；bypass 变体（整键覆盖/falsy/标量/snake_case/float floor/`texture: 1`）在测试内**程序化派生**自该副本（G0.9：不手写 fixture）；audit trail 的合并 rationale 上限 500 字（防 trail 行不可 grep）
