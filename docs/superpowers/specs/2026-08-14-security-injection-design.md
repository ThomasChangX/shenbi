...
- 修复：(a) skill 名词法校验单一入口（`^[a-z0-9][a-z0-9-]*$` 白名单，拒绝 `/`、`..`、空），dispatch_helper/legacy/_skill_path/phase_runner 三拼接点与 `load_contract` 复用同一校验函数（74 个现有 skill 名已全量核对零误拒）；(b) generate.py output 路径 `resolve()` 后 `is_relative_to(REPO_ROOT)` 校验，逃逸即报错
