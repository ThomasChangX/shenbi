...
- 修复：`python3 -c` 调用改 argv 传参（`python3 -c '…' "$ROUND_DIR"` + python 侧 `sys.argv[1]`）或等价参数化（本文件 JSON 解析已全用 python json，无 grep -o 面——该条款为自 run_pipeline.sh 漂移，不适用；argv 化后值须经 `json.dump` 序列化写入，顺带修复 EXPECTED_CHAPTERS=N/A 未加引号破坏 progress.json 的隐性 bug，不得用字符串模板回写保留旧形态；该附带修正位于 create 模式，无 CI 覆盖面，标注为人工验证项）
