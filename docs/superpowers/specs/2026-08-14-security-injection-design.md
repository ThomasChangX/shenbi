...
- 修复：`python3 -c` 调用改 argv 传参（`python3 -c '…' "$ROUND_DIR"` + python 侧 `sys.argv[1]`）或等价参数化（本文件 JSON 解析已全用 python json，无 grep -o 面——该条款为自 run_pipeline.sh 漂移，不适用）
...
- 修复：(a) skill 名词法校验单一入口（`^[a-z0-9][a-z0-9-]*$` 白名单，拒绝 `/`、`..`、空），dispatch_helper/legacy/_skill_path/phase_runner 三拼接点与 `load_contract` 复用同一校验函数；(b) generate.py output 路径 `resolve()` 后 `is_relative_to(REPO_ROOT)` 校验，逃逸即报错
