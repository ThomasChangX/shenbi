...
- ✅ 加配置文件（pyproject.toml / ruff / mypy / pyright / pre-commit / CI）
...
7. pre-commit hooks（ruff/mypy/pyright + registry 自动 rebuild）
...
5. **构建产物 gitignored**。__pycache__/、.coverage、htmlcov/、docs/api/、tests/coverage/ 不入 git。
...
#### PR 1: pyproject.toml + uv.lock + 项目元数据
...
dev = [
    ...
    "hypothesis>=6.97.0", "mypy>=1.8.0", "pyright>=1.1.343",
...
`tests/conftest.py` 全局 fixtures：configure_logging / tmp_project_dir / sample_worldbuilding_output / mock_subagent_dispatcher。
  ...
  - repo: local
    hooks:
      - id: registry-lockfile-fresh
        ...
        entry: uv run python tests/build_registry.py
        language: system
...
JSON 输出（生产）+ console 输出（dev），环境变量 `SHENBI_LOG_FORMAT=json|console`。每次工具调用 = 一个 event，含结构化字段。替换所有 print（约 75 处）。
...
#### PR 10: typed exception hierarchy
...
| 类型检查 | mypy + pyright (双重) |
| 测试框架 | pytest + 6 插件 |
| 覆盖率门槛 | 90% line / 80% branch |
...
Developer → Edit → git add
                      ↓
                  pre-commit hooks
                  (ruff / mypy / pyright / check-yaml / detect-private-key)
                      ↓ (若失败，回到 edit)
                  git commit
                      ↓
                  git push
...
- B. 类型检查与 lint 自身（subprocess 调用 ruff/mypy/pyright）
- C. 异常类与 logging
...
| 4 | pyright config | PR1 | Type check |
...
| 15 | rename novel-output → skill-output | — | Cleanup |
...
P0 在 P-1 上加：
  ...
  - skills/<name>/schema/*.py（Pydantic，受 mypy/pyright 监督）
...
### Risk 4: mypy/pyright 在现有代码上报大量错误
...
- 部分 PR 由 AI 自动 review（ruff/mypy/pyright 已是机器检查，review 重点是设计决策）
...
└── tests/
    ...
    ├── unit/                            (~780 tests 最终)
    ├── integration/                     (~150 tests 最终)
    ├── property/                        (~50 tests 最终)
    ├── benchmark/                       (~20 tests 最终)
    ├── fixtures/
    │   ├── sample-novel.json
    │   ├── sample-worldbuilding-output/
    │   └── sample-character-design-output/
