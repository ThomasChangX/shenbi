...
1. `pyproject.toml` + `uv.lock` + `.gitignore` + `renovate.json`（含 dev 依赖与 hashes）
...
14. 目录重命名 `novel-output` → `skill-output`（PR-12 之后，含 10 处引用更新）
...
**pytest-xdist + timeout + benchmark 兼容性**：pytest-xdist (`-n=auto`) 与 pytest-timeout (`--timeout=60`) 协同时，per-test timeout 在每个 worker 内独立计算。Hypothesis profile=ci（1000 examples）与 benchmark 在 60s 内可能紧张。CI 跑 property/benchmark 时建议：

```bash
pytest -p no:cacheprovider -m "property or benchmark" --timeout=300
```

或拆分慢测试到独立 CI job。

**pytest-xdist 与 `@pytest.mark.last` 冲突解决**：xdist (`-n=auto`) 将测试分发到多个 worker，**忽略 `@pytest.mark.last` 顺序保证**。这意味着 `test_branch_coverage_meets_threshold` 可能在 `coverage.xml` 还未生成时就被某个 worker 执行，导致 CI 偶发失败。

CI 必须拆分为两个 pytest 调用：

```bash
# Step 1: 主测试（并行），排除 last marker
...
**范围说明**：仅替换 Python `print()`。Shell 脚本（`tests/dispatch-subagent.sh`、`tests/round-exec.sh` 等）的 `echo` / `printf` 用于 pipeline 控制与用户进度反馈，不是诊断日志，**不在 structlog 替换范围**。Shell 脚本如果需要结构化日志，PR-13 的 Python 重写后会自然使用 structlog。
...
- 0007-adr-process.md（meta-ADR：记录"使用 ADR 流程"的决策。可改为 `docs/adr/README.md` 而不占编号；P-1 内选择保留编号以与其他 ADR 一致。）
...
**缓解**：
- uv 是单文件二进制，安装简单
- 提供 fallback：`uvx --from shenbi <command>` 或 `pip install -e . --no-build-isolation`（注意 hatchling 在某些 Python 版本上 editable 模式对 namespace 包有边缘问题，需 `--no-build-isolation`）
...
**风险**：每个 PR review + merge 平均 2 天。**关键路径**（不可并行的最长链）：

```
PR-1 → PR-2/3/4（并行）→ PR-5 → PR-6 → PR-7 → PR-12 → PR-13/15（并行）
```

= 7 个串行步骤 × 2 天 = **14 天关键路径**（不是 34 天，34 天假设全串行）。

加上可并行的 PR-8/9/10/11/14/16/17/18/19（每个独立 ~2 天，可同时进行），实际总周期 ~14-18 天。

**缓解**：
- 关键路径上的 PR 优先 review
- 无依赖的 PR 并行 review（PR-11 ADR / PR-14 archive / PR-17 mkdocs / PR-16 security 完全独立）
...
**风险**：matrix testing（2 OS × 2 Python × 1 job = 4 并发 run）每 PR 4 个 run。GitHub Actions 价格差异显著：

- ubuntu-latest: $0.008/min
- macos-latest: $0.08/min（约 10 倍）

按 10 min/run 计算：

- 2 Ubuntu runs: 2 × 10 × $0.008 = $0.16
- 2 macOS runs: 2 × 10 × $0.08 = $1.60
- **合计 ~$1.76/PR**（不是简单 4 × $0.08）

**缓解**：
- 公共仓库免费配额充足（2,000 分钟/月，macOS 计 10× 配额）
- 私有仓库成本：若 50 PR/月，~$88/月。预算敏感场景可：
  - 仅在 main 分支 push 时跑 macOS（PR 仅 Ubuntu）
  - 完全去掉 macOS（接受 Mac 用户本地跑测试）
- 长期压缩成本：去掉 macos-latest（Linux only），保留双 Python 版本
