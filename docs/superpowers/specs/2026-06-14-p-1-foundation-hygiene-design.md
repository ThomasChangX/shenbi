...
pytest-cov 的 `--cov-fail-under=N` 仅作用于 line coverage。Branch coverage 阈值（80%）通过额外步骤强制：
...
def test_branch_coverage_meets_threshold():
    ...
    if not COVERAGE_XML.exists():
        pytest.fail("coverage.xml not found; run pytest with --cov-report=xml")
...
CI 在 pytest 之后自动跑此测试（无需额外步骤，它本身就是 pytest 测试）。
...
repos:
  ...
  - repo: local
    hooks:
      ...
      # 标记为 always_run + verbose 是为了让它每次 commit 时打印"placeholder"提醒。
