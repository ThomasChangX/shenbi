...
> **系列:** 2026-08-15 全项目深度审计 · 阶段 5（簇 C9，18 条）| **代表 finding:** F134 | **严重度上限:** P1（F202/F232/F603/F818）| **涉及文件面:** src/shenbi/scoring.py（classify）、contracts/skills/genre_config.py、pacing_design.py、config/config_coherence.py、gates/（g3、g0、g4）、thresholds.py、genre-config.json、约 10 个技能 SKILL.md 阈值表
...
2. **键集多源**：F214（_GENRE_KEYS 声称 9 键含 tropeInventory，实际 genre-config.json 8 键，键集三处平行定义）、F603（resonance_global_floor float 绕过 Rule2 下限校验，P1 verified）、F822（备份文件名 .bak vs .bak.YYYYMMDD 两处不一致）。
3. **承诺规则未编码**：F232（genre-config 契约"9 条可自动检查规则"只实现 7 条——approval 必填与顶层字段数=8 完全未编码，G4 放行无审批配置，P1）、F202（禁用维度 + 空 customRules 绕过规则 7 校验——G4 洞，P1 verified）、F436（G0.cc 的 E11 阈值一致性检查生产死线：gate_G0 调 check_config_coherence 从不传 resonance_global_floor）。
...
- **T2 · genre-config 契约补全（F202/F232/F214/F822/F603）**：补编码 approval 必填 + 顶层字段数校验；禁用维度/空 customRules 不再绕过规则校验；_GENRE_KEYS 以真实 genre-config.json 键集生成（或加 lint 对账）；Rule2 下限校验对 float 生效；备份文件名单源。
- **T3 · 死线校验接线（F436）**：gate_G0 → check_config_coherence 传参修复（resonance_global_floor 等 E11 面全部字段），E11 检查生产生效。
...
1. `python3 -c "..."` 复算 F232 口径：genre-config 9 条规则全编码（无审批配置的 fixture → G4 FAIL）；F202 口径：禁用维度 fixture 不再绕过。
2. `uv run shenbi-validate G0 <seed>` 后 config coherence 检查覆盖 resonance_global_floor（F436 断言：构造越下限配置 → FAIL）。
...
F134 F202 F213 F214 F232 F411 F436 F443 F603 F760 F806 F808 F818 F820
F822 F843 F846 F875
