...
- **方案:**
  ...
  2. **Checksum 对比** (设计决策 #13): 读取 `state.last_snapshot` 中的 checksum manifest，对比当前 truth files 的 SHA256
     - 一致: 正常恢复
     - 不一致: 强制运行 `truth-sync` (从最近完整章节重新提取)，然后恢复
  3. 如无 `last_snapshot` (首次 resume，genesis 后无快照): 跳过 checksum 检查，仅做存在性检查
...
3. 如果 BLOCKING: 调用 `handle_audit_blocking` 检查重试计数:
   - 如果 `audit_retry_count < max_audit_retries`: 通过 **step_index 回退** 实现 revision 闭环:
     - 设 `step_index` 回退到 revision step (step 18) 的位置
     - 递增 `audit_retry_count`
     - 主循环自然重新执行 step 18 (chapter-revision) -> step 19-20 (snapshot/drift) ->
       再次到达审计 step -> 重新审计
     - 注意: snapshot/drift step 在 revision 后需要重新执行 (章节内容已变更)
   - 如果 `audit_retry_count >= max_audit_retries`: 进入 escalation 路径 (A4)
4. 如果无 BLOCKING: 正常推进到 review-resonance (step 9)

**step_index 回退机制说明:**
- chapter_loop 的主循环基于 `step_index` 线性推进
- 审计 BLOCKING 后不修改循环结构，仅修改 `step_index` 的值
- 设置 `step_index = revision_step_index - 1` 使下一个迭代从 revision step 开始
- `_advance` / `_record_step_done` 语义不变 (revision step 会被重新记录)
- 新增 `audit_retry_count` 到 `ChapterState` (可序列化，跨命令持久化)
