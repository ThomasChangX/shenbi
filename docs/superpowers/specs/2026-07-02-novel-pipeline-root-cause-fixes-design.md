...
- **方案:** 不使用硬编码的 skill 列表。`requires_independent()` 已读取 contract 的 `requires_independent_agent` 标志。修复方式：catch 异常时，先尝试读取 contract 标志；如果标志为 `True` 则返回 `True` (fail-closed)；如果标志为 `False` 或读取失败则返回 `False` (fail-open)。这覆盖所有声明 `requires_independent_agent: true` 的 skill，符合 §14 设计决策 #6。
