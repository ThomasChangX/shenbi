...
|------|------|---------------|----------------|
| `world/power_system.md` | 28,808B | `review-world-rules`、`review-group-factual`（均为 string 全文） | ❌ 零 fields |
| `outline/volume_map.md` | 26,334B | 10 skill（chapter-planning/foreshadowing-plant/foreshadowing-lifecycle/pacing-design/plot-thread-weaver/review-arc-payoff/score-volume/sequel-writing/volume-outlining/book-spine-init），全部 string | ❌ 零 |
...
- `review-world-rules` 不在章审计波（`chapter_loop.py` 零出现）——它是 foundation/独立审计入口。
...
| skill | body 依据 | 应声明 fields（模板 8 个内） | 量级 |
|-------|----------|-------------|------|
| `review-group-factual`（主链 step 9） | body :133「战力体系=不可逾越的天花板」、:141「检查角色等级上限、能力使用代价、跨等级对决规则」 | `[力量天花板, 代价机制, 跨级战斗参考, 能力边界]` | ~28.8KB → ~10-14KB |
| `review-world-rules`（foundation/独立审计入口） | body :90「检查角色等级是否在 power_system.md 等级表中存在」+ 天花板一致性 | `[等级表, 力量天花板, 能力边界]` | 同上量级 |

（原表 context-composing 行作废——已退出消费面；原 review-world-rules 行的 `代价机制`/`跨级战斗参考` 删去——body 零出现；补 `等级表`——body :90 点名。子集终值在 plan 阶段按 body 精读定稿。）
...
- G4 对两 skill 仍 PASS（G4 不校验 reads 面——驳斥 B7 核实；跑 G4 确认契约变更无副作用）。

### 3.3 度量（离线化修订）

改前 vs 改后，直调 `_build_skill_prompt('shenbi-<skill>', fixtures 树, ...)` 量 `len(system_prompt + user_prompt)` 字节与 `estimate_prompt_tokens`（`src/shenbi/cost/estimate.py` 纯函数）——power_system 项预期降 ~15-19KB/次 ×2 skill。**不触发真实 dispatch**（核心原则 8；TokenLedger 真实行需 dispatch，禁用）。
...
dispatcher 按 extractor 名调提取函数（volume_chapter → `_shared` 族的按章卷上下文形态）。**提取实现统一到 `_shared` 族**（与 context_assemble 同源、已有测试），不复活 `_extract_volume_chapter`（审计侧死代码，见 §2.1）。触面（驳斥 B7 核实清单）：`contracts/loader.py` `_normalize_read_item`（:72-87，今日静默丢弃未知键——须让 `extractor:` 进旁路，如 `read_extractors`，与 `read_fields` 同模式）+ `_validate`（:204-239）+ dispatch read 循环 + `scripts/lint_contract_fields.py`（:53-86，识别「有 extractor 无 fields」合法形态）+ Contract TypedDict（:52）。**G4 不在触面**（gates/g4 零 reads 校验）；`sync_contracts`/`lint_contract_prose` 消费归一化 string 不受影响（前提：loader 保持归一化）。
...
- volume_map 其余 8 个 string 读者（审计/汇总/续写面）本 spec 不动——单 spec 原子性，扩面待 chapter-planning 效果验证后另行处置。

### 4.4 验证（离线化修订）

- 直调 `_build_skill_prompt('shenbi-chapter-planning', fixtures 树, chapter=N)`：volume_map 内容从 ~26.3KB 降至当前卷节点（~500B-2KB）；chapter=None（genesis 形态）→ 全文（兜底）。
- `_shared` 族既有测试仍 PASS；新增 dispatch 循环级用例（提取器生效 + 兜底两分支）。
- G4 对 chapter-planning PASS。
- 度量同 §3.3 离线口径。
...
| §3 | 安全字段键集（模板 8 个）跨生产路径稳定 | 生产者模板（power-system SKILL.md:120-185）+ git 历史 xinghuo 真实文件 + fixture 三方对比（§2.2 已落证） |
| §3 | 声明的 field subset 覆盖 skill 实际需要 | 离线断言零 `field_filter_missing_fields` WARN + G4 PASS + body 精读复核（plan 阶段） |
| §4 | `_shared` 族提取的卷节点足够 chapter-planning 决策 | G4 PASS + 人工抽查 planning 输出质量（fixtures 驱动） |
...
| §3 power_system fields | review-group-factual + review-world-rules 声明 fields（模板 8 个内） | 低（field 不匹配有 WARN 逃逸门 + 全文兜底） | 零 WARN + G4 PASS + 离线字节下降 |
| §4 volume_map extractor | `extractor:` 契约字段接 `_shared` 族（chapter-planning） | 中（契约 schema 扩展：loader 旁路 + lint 识别；提取器复用已测实现） | 双分支断言 + G4 PASS + 离线字节下降 |
...
| power_system.md 发送体积（review-group-factual） | ~28.8KB 全文 | ~10-14KB（≤4 fields） | 直调字节 + estimate_prompt_tokens |
...
| G4（review-world-rules / review-group-factual / chapter-planning） | PASS | PASS | `just gate G4` |
...
2. **逃逸门 WARN 即缺陷（事件名更正）。** `filter_to_fields` field 不匹配返回全文 + `field_filter_missing_fields` WARN——实施后离线断言零该事件。**死线修复前「零 WARN」是假绿**（过滤不发生当然无事件）——§3.0 必须先行并单独验收。
