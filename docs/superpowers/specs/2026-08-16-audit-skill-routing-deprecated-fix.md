...
- **F815**（P1，**降级**）：四子项中 (d) 之半——bridge_tracker.md 写声明已由 PR #202（457450b5，spec #58 C20）补入 contract（writes: append_dedup key bridge_id + reads + 写纪律段），退出本 spec；存活子项：(a) description 非触发式（:3 "Combined foreshadowing lifecycle -- recall… track… plant… in a single call"）(b) lifecycle-states.md/hook-types.md 相对引用落在他 skill 目录（:64/:108，实际在 track/plant 目录内，lifecycle 自身目录仅 SKILL.md）(c) :84 "Set initial lifecycle_state to ACTIVE" 与输出示例 :161 PLANTED 及 :65 状态机起点矛盾——**且 ACTIVE 非 HookState 规范六态之一**（hooks.py:19-27：PLANTED/RELEVANT/TRIGGERED/RESOLVED/ARCHIVED/EXPIRED，解析器把 ACTIVE 折叠为 None——轮 3 复核加重） (d) 残半——audits/chapter-N-foreshadowing.md 输出（:140-142）仍不在 contract writes/updates
...
1. **using-shenbi 触发表**（F873/F834）：删 14 个 DEPRECATED 路由行（:44-54/:63/:73-74）；**删行不删语义**——被删行的用户触发短语（"连贯性/对不上"、"对话问题/台词"、"动机不合理"等）合并/改指到后继触发行（如"对话问题"→ group-character、"伏笔埋线"→ foreshadowing-lifecycle），防验收 2 字面满足而意图短语覆盖静默丢失；为 4 个 group-* 与 foreshadowing-lifecycle 补触发行；默认审计列 :124 同步——3 个 DEPRECATED 出列、后继进列；:126 条件审计 Phase 列表的 DEPRECATED 项同步清理或改指后继
...
11. 新增对账规则（**独立工具挂载**，设计审查轮 1 裁决：lint_contract_graph 是 reads/writes 闭包检查器，路由面规则塞入属语义拉伸——立 `tools/lint_routing_faces.py` 类独立工具挂 justfile check 链，与 C25 CI 承载协同）：(a) DEPRECATED skill 名出现在 deps.json/using-shenbi/GENESIS_STEPS/TRIGGER_STEPS/GENRE_ACTIVATION_MATRIX/CHAPTER_STEPS/**BOUNDARY_TRIGGERS** 任一路由面即 FAIL（设计审查轮 2 补 CHAPTER_STEPS——最直接的每章派发面，F905 之半即生于此；轮 3 补 BOUNDARY_TRIGGERS——同文件内真实派发面，现四项干净，防回潮近零成本；与 T1.3 的 lint_repo_consistency 豁免语义合一，单一信源——**DEPRECATED 判定须共享同一探测 helper**，防两份标志表漂移，helper 归宿 plan 定稿，标志 regex 契约随 DEPRECATED 正文形态钉死）(b) 触发表路由的每个技能必须存在于 skills/ 磁盘且非 DEPRECATED (c) description 契约（长度 + 触发式结构——接 T2.7 强化后规则源）
...
## 簇成员清单（12 条簇成员 + 1 条跨轮补登，自查用 · 2026-09-12 驳斥复核标注）
