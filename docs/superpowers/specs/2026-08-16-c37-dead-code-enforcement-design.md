...
## 目标
1. 每处存活面（~27，以 R0 现判表为准）一裁决：**接线**（移交对应簇 spec）/ **删除**（本 spec 执行）/ **显式 deferred**（登记理由与复活条件），零"无主"残留；已修 7 条记 "already-fixed by <spec>" 关闭行
2. 假防线优先：A 类全部消除（要么真接线要么删声称）
3. CI 引入 dead-code 执法（vulture 白名单制），防回归

## 任务分解
### R0 · 分桶裁决表（先于一切动作）
- 产出 `docs/superpowers/audit-runs/2026-08-15/c37-triage.md`：按**当前 main 现判**（非照抄 2026-08-16 名单）——原始 43 条 × 四桶（wire→认领簇 spec 编号 / delete→本 spec / defer→理由 / already-fixed→注明修复 spec）。**每 finding 编号一行（43 数据行，禁止区间合并行——验证命令按行计数）**。裁决规则：对应簇 spec 仍活跃的机制面交其认领（`_verify_truth_integrity`→C30 R2 若仍活跃）；**认领簇 spec 已归档时不再移交**——其裁决未覆盖的面按本 spec delete/defer 处置或登记新后续 spec（append-only 入 INDEX），归档 spec 不接收工作；无认领且近三轮无引用者删；有明确设计意图但无预算者 defer（≤5 条）；F319/F321 按重构后代码形态现判
- **验收**：表覆盖原始 43/43（含 already-fixed 关闭行，一编号一行）；每条有桶归属与执行者

### R1 · A 类假防线清除（优先）
- error_guidance/recovery（F108/F109）：整模块删除或最小接线（若 dispatcher 想要错误指引则接线+修 doc_url；默认删）；F378/F886：注释/文档改为真实描述，genesis-context 零消费问题移交 C30/C1 裁决（其内容断流是产品缺陷不只是卫生）
