...
decisions.json sidecar（Layer A）契约在 schema↔写方↔路由↔G2/G4 四层各自为政，生产 145 个 decisions.json 仅 5 个通过校验（F237：44 非法 JSON + 42 schema 违反；F1102 实证 88/145=61% 不合规）。四层断裂：
...
- **T1 · 路由修复（T101/F438/F205，P0）**：derive_file_type 改为逐产物类型（file-level 而非 skill-level）——同一技能的 .md 走章节检查、.json 走 decisions 检查；G2 对两类都校验（g2.py:83-84 无条件 continue 删除）。修复形状建议：派发产物清单携带 (path, file_type) 元组，替代技能级单一 file_type。
...
- 风险：T1 逐产物路由使此前"漏检"的 .md 突然进入 G2，存量产物可能批量 FAIL——先以 WARN 模式跑一轮全量盘点再启用 FAIL。T3 放宽读取 schema 时防止把"尾随拼接"合法化——raw_decode 恢复仅用于告警归档，不静默放行（与 C13 F507 联动）。
