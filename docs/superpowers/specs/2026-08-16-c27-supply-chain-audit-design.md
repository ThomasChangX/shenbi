...
- SBOM 生成改为按组分层：三份 `cyclonedx-py requirements -o <group>.cdx.json` 分别吃 prod / dev / docs 组的 `uv export --frozen --no-emit-project` 产物（prod=`--no-dev --all-extras`，dev=`--only-group dev`，docs=`--only-group docs`——默认 export 是 prod+dev，prod 必须显式 `--no-dev` 否则复造 F1041 过度包含；uv 不允许 `--only-group` 与 `--all-extras` 同用，extras 不属于 dev/docs 组，dev 组仍含 ML 栈因 sentence-transformers 是 dev 直接依赖）（`environment` 子命令无法按组分层）；docs/dev 组产物以属性/命名标注"不分发"。security.yml 与 release.yml:17-18（第二修复面，spec 范围字段原漏列）同步改，release.yml:46 `files:` 上传清单同步改三份分组 SBOM 文件名，SECURITY.md:25 "SBOM attached to Releases" 措辞同步
