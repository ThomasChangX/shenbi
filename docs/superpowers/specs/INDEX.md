...
- **状态**：Design（**Revised 2026-09-17** · SDD 阶段 3 六轮审查修订：T_B 缓存降级不实施——审计波已归 #42/PR#153，剩余纯 I/O；§3 重设计为无条件 body 瘦身——外置到各 skill 自有目录（先例 anti-ai-reference.md），运行时不注入，废弃 dispatched_examples 与 `skills/_shared/`；resonance/arc-payoff 阈值口径 SKIP（净省 <500B）+ state-settling 定性裁决 SKIP（588B 超阈值但无 G4 安全网输出契约风险不值）——重段主体为 checker/机器/输出契约结构——**实施集终版 2 skill（chapter-pattern/pacing），一次性 ~1.46KB、稳态 ~0.12KB/章；pass 主交付 = T_A 字节稳定测试 + T_D IDE 分离（CLI 门控）+ 分解证据**）
