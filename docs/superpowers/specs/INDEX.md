...
- **状态**：Executed 2026-08-15（分支 fix/spec-6-pipeline-never-completes，T1-T9 落地待 PR；此前 Revised · 设计审查 6 轮收敛 0C/0I：R1 卷级作用域+负面验收 / R2 total=100+在途 heal / R3 参数化目录校验+manifest 钉契约 / R4 per-step N 语义表+四消费者+[path-context] 跨路由通道 / R5 显式解析上下文 / R6 三消费方+全桥接段聚合 / F303 拆至 #26 / F340 重试计数重置+全类型表 / F341 全守卫体镜像 / F304 捕获定位 / F380 哨兵路径）
