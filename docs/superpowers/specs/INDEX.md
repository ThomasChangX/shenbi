...
- **状态**：Design | **优先级**：🟠 P1
- **内容**：上轮 verified 零承接（F1177）/ 跨轮 F 编号 72/123 碰撞（F978）/ ledger 畸形行与记账缺口（F972/F973/F975/F969/F1176）/ 不可复现声称与重复立案（F767/F768/F894）/ 孤儿分支与 INDEX 计数漂移（T1502/T1507）——audit-lint 三方对账 + 跨轮承接清单 + ID 命名空间隔离 + severity 校准 12 项核实注记（已由 PR #147 落账，phase4 §4）；全程可并行
