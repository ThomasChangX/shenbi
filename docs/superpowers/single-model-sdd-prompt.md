...
- **停机报备**（不自治继续）：① BLOCKING deviation ② CI 无法运行 ③ 连续 2 份 REJECT（说明注册表或 main 走向系统性漂移——本仓活跃 spec 多数源自 2026-08-14/15 审计系列，历史各轮来源分布以 specs/archive 目录现势为准；审查方法或覆盖若有系统性偏差会集中暴露，剩余队列裁决可信度存疑，需人工抽查后再放行）
