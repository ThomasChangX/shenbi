...
### Task 3: context-composing 改造（按层组装）
...
git commit -m "feat: rewrite context-composing for hierarchical layer assembly (spec §3.5, §3.2.1)"
```
...
在 `src/shenbi/gates/g4/__init__.py` 的 import 块添加：
```python
...
在 G4 router（查找 skill name → checker 映射）添加映射。
...
将 `tests/tiers/deps.json` 的 `genesis` phase prerequisites 末尾添加 `"shenbi-book-spine-init"`：
