...
def build_index(project_dir: Path | str) -> TruthIndex:
    ...
    # Index characters
    for card in (
        (project_dir / "characters").rglob("*.md") if (project_dir / "characters").exists() else []
    ):
        fm = _parse_frontmatter(card.read_text(encoding="utf-8"))
        name = str(fm.get("name", card.stem))
        idx.characters[name] = IndexEntry(
            "character", name, str(card.relative_to(project_dir)), f"characters/{card.name}"
        )
    # Index hooks
    ...
    if hooks_file.exists():
        ...
        for h in fm.get("hooks", []) if isinstance(fm.get("hooks"), list) else []:
            if isinstance(h, dict):
                hid = str(h.get("id", ""))
                idx.hooks[hid] = IndexEntry(
                    "hook",
                    hid,
                    "truth/pending_hooks.md",
                    f"truth/pending_hooks.md#{hid}",
                    {
                        "state": h.get("state", ""),
    ...
    if rules_file.exists():
        for m in re.finditer(
            r"^##\s+(R?\d+)[:.]?\s*(.+)$", rules_file.read_text(encoding="utf-8"), re.MULTILINE
        ):
            idx.rules[m.group(1)] = IndexEntry(
                "rule",
                m.group(1),
                "world/rules.md",
                f"world/rules.md#{m.group(1)}",
...
class TestAssembleContext:
    ...
    def test_write_context_file(self, project_with_plan):
        ...
        assert out.exists()
        assert "Spine" in out.read_text(encoding="utf-8") or "route" in out.read_text(
            encoding="utf-8"
        )
```
...
def main() -> int:
    ...
    if args.command in ("update", "rebuild") and args.text:
        ...
        embed_and_store(
            store,
            args.text,
            chunk_id=f"manual-{hash(args.text)}",
            source_file="manual",
            chunk_type="manual",
        )
    store.close()
