...
- [ ] **Step 2: Update contract reads for bootstrap mode**

Bootstrap mode reads `novel.json` and `genre-config.json` which are not in the current contract. Additionally, `chapters/*.md` won't exist during genesis. Update the contract reads:

```yaml
reads:
  - chapters/*.md              # normal mode (exists after ch1)
  - import/source/*.txt        # optional reference works
  - novel.json                 # for bootstrap mode
  - genre-config.json          # for bootstrap mode
```

The pipeline's ramp-up read-skipping (spec §17) will skip `chapters/*.md` during genesis bootstrap dispatch.
