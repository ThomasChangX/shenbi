...
7. **跨段重复立案须显式 merged 标注**：同一缺陷在多个段/线程被立案时，后立案条目必须在标题或注记中显式标 `merged-into-F<主条目>`（2026-08-15 F894 形态：11 个 .gitkeep 三段 6 立案 5 放行）；未标注的重复立案在 phase4 聚类时按记账缺口论处。
8. **"N tests"类声称必须附文件名与命令**：任何「N tests collected/passed」断言必须附所跑测试文件名与完整命令及输出（F768 形态："96 tests collected" 未指名文件不可复现）；只报数字不报来源 = 不可复现断言，按未验证论处。
...
*本 prompt（v3）与设计 spec `docs/superpowers/specs/archive/2026-08-13-full-project-audit-prompt-design.md` 及其 §0（v2 修订）/§0.1（v3 修订）一一对应；执行时如发现本 prompt 内部矛盾或缺失机制，停止并报人类裁决，禁止自行改写规则。*
