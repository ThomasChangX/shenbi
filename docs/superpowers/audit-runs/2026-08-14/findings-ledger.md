...
| F652 | detect_rhetoric「反复」指标按子短语分解计数：一个 7 字短语重复 3 次 → 反复=12（5+4+3 个子 3/4/5-gram 各计 1）——量级随短语长度膨胀且非重复次数本身 | 统计口径 | P2 | 见 Z6.review9.md#F652 | 见报告 | 见报告 | 见报告 | 见报告 | deep-read | verified |
| F253 | codex.py `_record_completion` 对 progress.json 做非原子读-改-写：并发 shenbi-dispatch 进程丢失完成记录（safe_write flock 仅覆盖写入不覆盖读取） | 漏报（并发竞态，被 F227/F235/F247/F252 链掩盖） | P2 | 见 Z2.review9.md#F253 | 见报告 | 见报告 | 见报告 | 见报告 | deep-read | verified |
...
| F158 | phase_runner 的 `phase` 参数未净化直接拼接进状态文件路径：`load_state`/`save_state` 双向路径穿越，可写出 round_dir 外任意位置（含覆盖既有文件），并造成跨 phase 状态文件读写混淆 | security/path-traversal | **P1** | 见 Z1.review12.md#F158 | 见报告 | 见报告 | 见报告 | 见报告 | deep-read | verified |
...
| F397 | state-settling 的 append_dedup/upsert 写语义在派发写路径是 no-op：累积 truth 文件（chapter_summaries/emotional_arcs/particle_ledger/subplot_board）每章被整文件覆写为仅当章内容，前章数据确定性丢失 | error | P0 | 见 Z3.review12.md#F397 | 见报告 | 见报告 | 见报告 | 见报告 | deep-read | verified |
...
| F3A2 | plan_skeleton 卷末章 `next_chapter = min(chapter+1, ch_end)` 自指：卷末章的 section 6 "Next chapter" 渲染为同一章（Ch{N} role 重复当前章节点） | error | M | 见 Z3.review14.md#F3A2 | 见报告 | 见报告 | 见报告 | 见报告 | deep-read | verified |
...
| F3A4 | `_append_integrity_findings` 对 `.integrity-findings-{N}.jsonl` 的无锁读-改-写：并行审计波两线程并发追加同一诊断文件时 lost-update（丢记录/交错覆盖） | error | M | 见 Z3.review14.md#F3A4 | 见报告 | 见报告 | 见报告 | 见报告 | deep-read | verified |
...
| F260 | `_changed_top_keys` 对**新增 null 值键**返回空变更集 → field 级 OWNERSHIP 键集校验对 `{"key": null}` 新增旁路 | 漏报（审计盲区，跨区根因：Z5 audit/snapshot.py，被 Z2 executor 审计链消费） | M | 见 Z2.review12.md#F260 | 见报告 | 见报告 | 见报告 | 见报告 | deep-read | verified |
...
| F659 | compute_stats.PUNCT_MAP 与 cjk.PUNCTUATION_TOKENS 双标点统计实现口径分叉：同一文本两模块给出系统性不同桶计数（感叹号 0 vs 4、冒号 5 vs 18、引号 72 vs 3、破折号 100 vs 25） | 跨模块双实现口径分叉（F607/F632/F646 同族） | P2 | 见 Z6.review12.md#F659 | 见报告 | 见报告 | 见报告 | 见报告 | deep-read | verified |
