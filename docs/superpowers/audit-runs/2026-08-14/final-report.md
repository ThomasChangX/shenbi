...
| 覆盖缺口（d1-06） | 1448 真实未覆盖行（85.16% cov 版本口径；coverage.xml 最终 line-rate 87.28%，2026-08-14 快照，修复 2 次污染事故后） |
...
| P2 | 496 | 边界/错误处理/统计错误/死代码/漂移 |
| M | 166 | 文案/命名/格式/过期注释 |
| false-positive | 1 | F325（复核反证撤销） |
| merged | 35 | 同根因去重合并 |

**总 findings: 781**（655 verified + 90 specced + 35 merged + 1 FP）
...
- 台账：coverage-ledger（G1 通过）/ findings-ledger（786）/ meta-audit.md（G6 通过）
