...
**可自动检查规则**：每卷必须填写三条线的实际百分比。CONSTELLATION 低于 15% 或高于 40% 触发警告（硬失败带 [15,40]，卷内细分以「目标比值（按卷型）」表为准）。
...
**检查规则**：
- PASS: QUEST 40-60%, FIRE 25-35%, CONSTELLATION 以「目标比值（按卷型）」表为准（硬失败带 [15,40]）
- WARN: 与目标偏差 ≤ 10%
- FAIL: 与目标偏差 > 10%
...
**可自动检查规则**：必须定义 6-12 种场景类型，单卷内不连续 3 章同类型。
...
- [ ] 定义了 6-12 种场景类型
- [ ] 每卷四拍完整（铺垫/升级/爆发/余波）
- [ ] 每卷三线比例在目标范围内（CONSTELLATION 以「目标比值（按卷型）」表为准，硬失败带 [15,40]）
