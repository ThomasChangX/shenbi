...
**字数最低要求**：从 `novel.json` 的 `target_words` 除以章节数计算每章最低字数。实际字数 < 最低字数 = FAIL。
...
| 每章字数 | ≥ floor（target_words / 章节数） | < floor |
...
**审计结果列格式**：`PASS数/6 <FAIL维度英文名> <结果>`。只写"通过" = 不合格。每章字数必须 ≥ floor（floor = target_words / 章节数）。
