...
7. **truth files 必须全部初始化** — spec §4 定义的 12 个 truth files 必须在 worldbuilding 阶段全部创建为空模板。遗漏任何一个 = 后续 skill 写入失败 = 状态漂移。
