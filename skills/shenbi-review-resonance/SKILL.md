...
每维度报置信度（high/mid/low）+ overall。置信度校准与修订分流由框架确定性执行（原 §5.4 三路径 helper 已删除——生产路由权威是 `pipeline/revision_router`，锚点命中率校准派发后自动运行）。> 设计意图：只有**边界/不确定**走人因（防主观噪声假阻断死循环）；**明确失败**直接修订（高置信度 + 大幅低于阈值 = 真实缺陷，无需人因把关）。人因只在噪声区介入，不作每章瓶颈。
...
判定与共振分数以下列机器围栏块输出（置于本小节末尾；围栏外的判定/分数行不会被框架采纳；证据引述一律 `> ` 前缀且禁止行首裸三反引号）：
...
| 「评不过就多修几次总会过」 | 同章累计修订上限 2 次（一切修订成因共计，框架 revision_router 强制），第 3 次仍 clear-fail 必须人因介入，禁止无限重试 |
