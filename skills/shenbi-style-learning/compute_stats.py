...
# Sentence-ending punctuation across Chinese and mixed text
SENT_ENDS = re.compile(r'[。！？；\n]')
# Punctuation marks to count
PUNCT_MAP = {
    "句号": "。",
    "逗号": "，",
    "感叹号": "！",
...
def segment_sentences(text):
    ...
    for ch in text:
        ...
        if ch in "。！？\n":
            ...
            current = []
    # trailing text without sentence end
    if current:
        sent = "".join(current).strip()
        char_count = len(sent.replace("\n", "").replace(" ", ""))
        if char_count > 0:
...
def compute_sentence_stats(sentences):
    ...
    # Build histogram (buckets: 1-10, 11-20, 21-30, 31-50, 51-80, 81+)
...
def compute_paragraph_stats(paragraphs):
...
def compute_ttr(text):
    ...
    Also compute sliding-window TTR (1000-char windows) and content-word TTR.
    """
    # Strip whitespace and punctuation for character-level TTR
    chars = [c for c in text if c.strip() and c not in "。，！？；：""''「」『』（）——……、\n"]
    if not chars:
        return {"global_ttr": 0, "sliding_ttr_mean": 0, "sliding_ttr_std": 0}
    # Global character TTR
    unique = len(set(chars))
    total = len(chars)
    global_ttr = round(unique / total, 4) if total > 0 else 0
    # Sliding window TTR (window = 1000 chars, step = 500)
    window = 1000
    step = 500
    window_ttrs = []
    ...
    # Content character TTR (exclude common function characters)
    function_chars = set("的了是就在也和着地得过都会可以能没被把让对从到向与而因所以然则之其以此或但若虽非更很太只个些还又要去来说")
...
def compute_ngrams(text, n=2, min_count=5, max_results=30):
    """Extract character-level n-grams from Chinese text."""
    # Clean: keep only Chinese characters, punctuation, and newlines
    cleaned = []
    for c in text:
        if c.strip():
            cleaned.append(c)
...
def detect_rhetoric(text):
    ...
    results = {}
    # Parallelism (排比): 3+ consecutive sentences with similar structure
    ...
    for i in range(len(sent_texts) - 2):
        ...
        # Check if sentence lengths are similar (±30%)
    ...
    results["反问"] = fanwen
    # Repetition: same phrase appearing within 100 chars
    repetition = 0
...
def count_transition_words(text):
    ...
    for word in TRANSITION_WORDS:
        ...
        if count > 0:
            results[word] = count
    # Compute density: transitions per 3000 chars
    total = sum(results.values())
    density_per_3000 = round(total * 3000 / total_chars, 1) if total_chars > 0 else 0
    return {"words": results, "total_transitions": total, "density_per_3000_chars": density_per_3000}


def read_chapters(paths):
...
def compute_all_stats(texts):
    ...
    return {
        ...
        "bigrams": [{"gram": g, "count": c} for g, c in compute_ngrams(combined, n=2, min_count=50, max_results=20)],
        "trigrams": [{"gram": g, "count": c} for g, c in compute_ngrams(combined, n=3, min_count=20, max_results=20)],
        "4grams": [{"gram": g, "count": c} for g, c in compute_ngrams(combined, n=4, min_count=10, max_results=20)],
...
def main():
    if len(sys.argv) < 2:
        print("Usage: compute_stats.py <chapter_dir_or_files...> [--output stats.json]")
        print("  Outputs JSON to stdout if no --output specified.")
        sys.exit(1)
    args = sys.argv[1:]
    output_path = None
    if "--output" in args:
