...
| Excuse | Reality |
|--------|---------|
| "这段原文太精彩，整段抄进锚点" | 锚点存储工艺分析，不存储原文。版权边界没有例外 |
| "这个维度不好归类，随便放一类" | 9 类槽位映射是强制契约。归不进去说明该段落不适合做锚点 |
| "校准区间写得宽松一点更灵活" | 模糊区间 = 无法校准 = 锚点失效。必须给出 88-97/75-87/<75 三档具体基准 |
| "source_ref 大概标到作品名就够了" | 无法定位的分析无法复核。必须标注作品名 + 卷/章/场景精确位置 |
| "凭印象写分析，不用细读原文" | 没有逐句精读的工艺分析 = 虚构 = 污染整个评分基准 |
