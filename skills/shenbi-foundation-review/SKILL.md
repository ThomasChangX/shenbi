...
digraph foundation_review {
    ...
    "Read outline/story_frame.md" -> "Score: core conflict (25pts)";
    "Score: core conflict (25pts)" -> "Score: opening rhythm (20pts)";
    "Score: opening rhythm (20pts)" -> "Score: world consistency (20pts)";
    "Score: world consistency (20pts)" -> "Score: character distinction (15pts)";
    "Score: character distinction (15pts)" -> "Score: foreshadowing potential (10pts)";
    "Score: foreshadowing potential (10pts)" -> "Score: anti-trope/originality (10pts)";
    "Score: anti-trope/originality (10pts)" -> "Total ≥ 80?";
...
| 评分项 | 满分 | 得分 | 证据引用 | 裁判理由 |
|--------|------|------|---------|---------|
| 冲突清晰度（对抗双方+赌注明确） | 8 | X | [文件:段落] | [≤1句] |
| 冲突推动力（冲突驱动主角行动而非被动卷入） | 7 | X | [文件:段落] | [≤1句] |
| 私人关联（冲突与主角有个人层面的利害关系） | 6 | X | [文件:段落] | [≤1句] |
| 三级递进（显性→潜在→哲学三层冲突均已定义） | 4 | X | [文件:段落] | [≤1句] |
| **小计** | **25** | **XX** | | |
...
| 评分项 | 满分 | 得分 | 证据引用 | 裁判理由 |
|--------|------|------|---------|---------|
| 声音区分（≥3主要角色语音/措辞/节奏可互盲辨） | 6 | X | [文件:段落] | [≤1句] |
| 动机独立性（每个主要角色有独立于主角的目标） | 5 | X | [文件:段落] | [≤1句] |
| 缺陷真实（每个主要角色有影响决策的真实缺陷） | 4 | X | [文件:段落] | [≤1句] |
| **小计** | **15** | **XX** | | |
...
**评分程序：** 读取 `genre-config.json` 的 `tropeInventory`，将 `outline/story_frame.md` 的弧节拍对照每个套路的 `signatures`；命中数 > `overuse_threshold` → 套路依赖度扣分，并列出命中的节拍 + `rewrite_hint`。
