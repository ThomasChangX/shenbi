...
3. **B 线（中线）必有窗口** — 每条 B 线必须有时点明确的推进窗口
...
- 默认值：P0 max_gap=2, P1 max_gap=4, P2 max_gap=8, P3 max_gap=16
...
**输出文件**: 必须写入单一文件 `outline/thread_map.md`。不允许拆分为多个文件。
...
|----|------|--------|------|---------|--------|--------|------|
| A1 | [线索名] | P0 | 长线 | 2 | N | M | PLANTED |
| A2 | [线索名] | P0 | 长线 | 2 | N | M | RELEVANT |
| B1 | [线索名] | P1 | 中线 | 4 | N | M | RELEVANT |
| B2 | [线索名] | P1 | 中线 | 4 | N | M | PLANTED |
| C1 | [线索名] | P2 | 短线 | 8 | N | M | RESOLVED |

**列校验规则**:
- `优先级`: 仅允许 P0 / P1 / P2 / P3
- `类型`: 仅允许 长线 / 中线 / 短线
- `状态`: 仅允许 PLANTED / RELEVANT / PAYING_OFF / RESOLVED
- max_gap 默认值：P0=2, P1=4, P2=8, P3=16
- 所有列必须非空
...
使用以下 EXACT 列名，对每条 P0/P1 线索逐章检查：
...
|--------|------|----------|
| 空白章节数 | = 0 | > 0（任意一章无主推） |
| P0 max_gap 违规 | 全部 ≤ 2 | 任一 P0 实际值 > 2 |
| P1 max_gap 违规 | 全部 ≤ 4 | 任一 P1 实际值 > 4 |
| C 线完结 | 全部 RESOLVED | 任一 C 线在终点章后非 RESOLVED |
| 列完整性 | 线索总览表 8 列全部填充 | 任一空值 |
| 状态有效性 | 仅允许 4 种状态值 | 使用了不允许的值 |
...
- [ ] 每章有 ≥ 1 条线索推进（空白章节 = 0）
- [ ] 所有 P0 max_gap 实际值 ≤ 2（无 Y 标记）
- [ ] 所有 P1 max_gap 实际值 ≤ 4（无 Y 标记）
- [ ] 所有 C 线已 RESOLVED
- [ ] 线索总览表 8 列全部非空且值有效
- [ ] P1 线索高潮窗口已规划
- [ ] 交叉点类型分布（汇合/分离/缠绕/反差）均有覆盖
