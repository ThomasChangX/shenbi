...
# shenbi.contracts.loader.REGISTRY_PATH to this tmp file, so they are fully isolated
...
def _setup(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, name: str, body: str) -> None:
    ...
    monkeypatch.setattr("shenbi.contracts.loader.SKILLS", tmp_path)
    monkeypatch.setattr("shenbi.contracts.loader.REGISTRY_PATH", reg)
...
class TestDecisionsRegistryPaths:
    ...
    def test_context_decisions_json_resolves(self) -> None:
        from shenbi.contracts.loader import load_registry, resolves
