...
## Test Setup
A novel project exists with a drafted chapter at `drafts/chapter-7.md` (5000 words). The target chapter length is 4000 words. The agent ran length normalization and produced `drafts/chapter-7-normalized.md` with a consistency checklist at `reports/chapter-7-normalize-report.md`.

## Scenario
The length normalization has been completed. The normalized version at `drafts/chapter-7-normalized.md` achieves the target word count (approximately 4000 words). However, the normalization added a new scene that did not exist in the original:
...
|----------|--------|-------------------|
| `drafts/chapter-7-normalized.md`: teahouse scene (between market and meeting scenes) | Narrative changed — new scene added that was not in the original draft, introducing new plot events and information | error |
| `reports/chapter-7-normalize-report.md`: full document | Missing consistency checklist — required confirmation of no narrative changes is absent | error |
