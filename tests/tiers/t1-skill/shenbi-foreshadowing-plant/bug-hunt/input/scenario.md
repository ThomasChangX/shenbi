...
## Test Setup
A novel project exists with foreshadowing planting output for chapter 5. The planting summary in `tests/fixtures/pending-hooks-example.md` lists 12 new foreshadowing operations planted in chapter 5: 7 plant operations, 3 reinforce operations, and 2 trigger operations. The skill's hard limit is 8 operations per chapter.
...
|----------|--------|-------------------|
| `tests/fixtures/pending-hooks-example.md`: chapter 5 planting section | 12 foreshadowing operations in chapter 5, exceeding the ≤8 operations per chapter budget (7 plant + 3 reinforce + 2 trigger = 12) | error |
