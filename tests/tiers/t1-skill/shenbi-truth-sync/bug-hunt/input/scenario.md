...
## Test Setup
A novel project has chapters 1-18 completed. The truth sync scope is set to chapters 15-18. The truth file at `tests/fixtures/character-profile-example.md` records:
...
|----------|--------|-------------------|
| `tests/fixtures/report-example.txt`: weapon field for Chen Wei | Missed conflict — chapter 17 states Chen Wei discarded twin daggers and acquired longsword "Dawnbreaker", but truth-sync output does not flag this contradiction with `tests/fixtures/character-profile-example.md` which still lists "Twin daggers, named Shadow and Shade" | error |
