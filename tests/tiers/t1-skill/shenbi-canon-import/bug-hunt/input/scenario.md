...
1. **Silent deviation**: The character canon at `tests/fixtures/character-profile-example.md` changes the protagonist's core motivation from the original work's public-duty orientation to the self-serving goal "贪财好色的躺平生活". This is a significant deviation from the original character.
   The deviation is NOT declared in the deviation list at `tests/fixtures/report-example.txt`. In AU mode, deviations are allowed but MUST be explicitly declared.

2. **Missing source citation**: The event canon at `tests/fixtures/report-example.txt` lists the school expulsion event involving "保尔·柯察金" but does not cite the original episode number or chapter reference. Three other event entries also lack source citations.
...
|----------|--------|-------------------|
| `tests/fixtures/character-profile-example.md`: protagonist motivation | Silent deviation — core motivation shifted to "贪财好色的躺平生活" without declaration in deviations.md | error |
