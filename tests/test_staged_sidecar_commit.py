...
def test_auto_state_settle_commit_copies_sidecar(tmp_path: Path):
    """The auto-commit loop copies staged sidecar JSON to live truth/."""
    from shenbi.pipeline.chapter_loop import staged_decisions_targets  # noqa: F401

    st = tmp_path / "staging" / "truth"
    st.mkdir(parents=True)
    (st / "current_state.md").write_text("x", encoding="utf-8")
    (st / "state-settling-decisions.json").write_text("{}", encoding="utf-8")
    # mirror the auto-commit copy loop's selection logic contract: the glob
    # must pick the sidecar up
    picked = sorted(st.glob("*.md")) + sorted(st.glob("*decisions*.json"))
    assert (tmp_path / "staging" / "truth" / "state-settling-decisions.json") in picked
